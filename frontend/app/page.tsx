...
export default async function Home() {
  ...
  const destaquePrincipal = posts[0];
  const ultimasNoticias = posts.slice(1, 5);
  const resenhasECriticas = posts
    .filter((post) => post.contentType === 'Resenha' || post.contentType === 'Crítica')
    .slice(0, 4);
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8 font-sans bg-black min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
        ...
        {destaquePrincipal && (
          <Link 
            href={`/post/${encodeURIComponent(destaquePrincipal.slug?.current ?? destaquePrincipal._id)}`} 
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-col gap-4 flex-1 justify-between">
            {ultimasNoticias.map((post: PostSummary) => (
              <Link key={post._id} href={`/post/${encodeURIComponent(post.slug?.current ?? post._id)}`} className="flex gap-4 items-center group bg-gray-950 p-2 rounded hover:bg-gray-900 transition-colors">
                {post.mainImage ? (
                  <Image
